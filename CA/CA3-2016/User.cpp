...
void User::endReading() {
	if (titleReading.first != "" && titleReading.second != "") {
		titlesRead.push_back(
				make_pair(titleReading.first, titleReading.second));
	}
}
...
bool User::operator<(const User& u1) const {
	...
	int thisNumber = 0, u1Number = 0;

	thisNumber += this->titlesRead.size();
	u1Number += u1.titlesRead.size();

	if (this->titleReading.first != "" && this->titleReading.second != "")
		thisNumber++;
	if (u1.titleReading.first != "" && u1.titleReading.second != "")
		u1Number++;

	return thisNumber < u1Number;
}
